...
$results = $db->viewSalePersonList($glid);

$page_url = $_SERVER['PHP_SELF'];
$input = json_encode(array("auth_username"=>$auth_uname,"auth_passwd"=>$auth_passwd,"v_code"=>$app_version,"device"=>$device,"model"=>$model,"pid" => $glid));	
$applog_data = array(
			'service_url' => $page_url,
			'request'	=> $input,
			'response' => $results,
